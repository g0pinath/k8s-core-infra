param($product_type, $critical_product="true", $key_product="true", $baseURL)
$Uri = "$baseURL" + "/api/v2/product_types/" 
...
$ProductTypeURI = $baseURL + "/api/v2/product_types/?name=$product_type"
$ProductTypeURI
$result = (Invoke-WebRequest -Uri $ProductTypeURI  -Method 'GET' -Headers $headers -SkipCertificateCheck).Content
$prod_id_count = ($result | Convertfrom-Json | select count).Count
